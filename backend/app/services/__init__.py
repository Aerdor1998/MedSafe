...
def __getattr__(name: str) -> Any:
    if name in {"DrugInteractionService", "get_interaction_service"}:
        ...
        return DrugInteractionService if name == "DrugInteractionService" else get_interaction_service

    if name in {"normalize_str", "patient_completeness", "build_recommendations_from_state", "compute_accuracy"}:
    ...
    if name in {"AnalysisOrchestrator", "get_orchestrator"}:
        ...
        return AnalysisOrchestrator if name == "AnalysisOrchestrator" else get_orchestrator
