...
class ClinicalRulesAgent:
    ...
    def __init__(self):
        ...
        logger.info(
            "⚕️  ClinicalRulesAgent inicializado - Base de 191k+ interações carregada"
        )
        ...
        try:
                    ...
                    interactions_found.append(
                        {
                            "interacting_drug": (
                                interaction["drug2"]
                                if interaction["drug1"].lower()
                                in medication_name.lower()
                                else interaction["drug1"]
                            ),
            ...
            if conditions or allergies:
                raw_contraindications = (
                    self.interaction_service.analyze_contraindications(
                        medication_name, conditions, allergies
                    )
                )
            ...
            risk_level = self.interaction_service.calculate_overall_risk(
                interactions_found, contraindications_found
            )

            # Ajustar risco baseado em fatores de risco do paciente
            if patient_risk_factors["critical_risk_count"] >= 2:
                risk_level = "critical"
            elif patient_risk_factors["critical_risk_count"] >= 1 and risk_level in [
                "low"
            ]:
                risk_level = "high"
            ...
            confidence = (
                0.85 if (interactions_found or contraindications_found) else 0.65
            )
            ...
            return {
                ...
                "evidence_links": (
                    [e.get("source", "Database") for e in evidence_snippets]
    ...
    def _evaluate_patient_risk_factors(
        ...
        for condition in conditions:
            ...
            if "renal" in condition_lower or "rim" in condition_lower:
                patient_profile.extend(
                    ["insuficiência renal", "insuficiência renal prévia"]
                )
        ...
        for reaction in adverse_reactions:
            ...
            for patient_factor in patient_profile:
                for reaction_factor in risk_factors:
                    if (
                        patient_factor.lower() in reaction_factor.lower()
                        or reaction_factor.lower() in patient_factor.lower()
                    ):
    ...
    def _check_dosage_adjustments(
        self, medication: str, age: int, weight: Optional[float], conditions: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        if any(
            cond in " ".join(conditions).lower() for cond in ["renal", "rim", "kidney"]
        ):
            adjustments.append(
                {
        ...
        if any(
            cond in " ".join(conditions).lower()
            for cond in ["hepática", "liver", "fígado"]
        ):
            adjustments.append(
                {
    ...
    def _get_common_adverse_reactions(self, medication: str) -> List[Dict[str, Any]]:
                ...
                [
                    {
                        ...
                        "severity": "Grave",
                        "risk_factors": [
                            "uso concomitante de outros serotoninérgicos",
                            "tramadol",
                            "IMAOs",
                        ],
                    },
        ...
        # === BENZODIAZEPÍNICOS ===
            ...
            med in med_lower
            for med in ["diazepam", "clonazepam", "alprazolam", "rivotril", "valium"]
        ):
            reactions.extend(
                [
                    {
        ...
        # === PARACETAMOL/ACETAMINOFENO ===
            ...
            med in med_lower for med in ["paracetamol", "acetaminophen", "tylenol"]
        ):
            reactions.extend(
                [
                    {
        ...
        # === ANTIBIÓTICOS (QUINOLONAS) ===
            ...
            med in med_lower for med in ["levofloxacin", "ciprofloxacin", "norfloxacin"]
        ):
            reactions.extend(
                [
                    {
                    ...
                    },
                    {
                        ...
                        "risk_factors": [
                            "cardiopatia",
                            "uso de outros QT prolongadores",
    ...
    def _generate_analysis_notes(
        ...
        if interaction_count > 0 or contraindication_count > 0:
            ...
            if contraindication_count > 0:
                notes.append(
                    f"- **{contraindication_count}** contraindicação(ões) detectada(s)"
                )
        ...
        if age >= 65 or age < 18 or pregnant:
            ...
            if age >= 65:
                notes.append(
                    "- **Paciente idoso**: Requer atenção especial para dosagem"
                )
            if age < 18:
                notes.append(
