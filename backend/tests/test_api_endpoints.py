...
def test_analyze_with_interactions():
    ...
    response = client.post(
        "/api/analyze",
        data={"patient_data": json.dumps(patient_data), "medication_text": "Aspirin"},
    )
...
def test_upload_image_endpoint():
    ...
    response = client.post(
        "/api/upload-image", files={"file": ("test.png", img_bytes, "image/png")}
    )
...
def test_invalid_patient_data():
    ...
    response = client.post(
        "/api/analyze",
        data={"patient_data": "invalid json", "medication_text": "Paracetamol"},
    )
...
def test_missing_medication_text():
    ...
    response = client.post(
        "/api/analyze", data={"patient_data": json.dumps(patient_data)}
    )
