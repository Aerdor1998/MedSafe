...
class VisionAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        logger.info(
            f"👁️  VisionAgent initialized with model: {self.vision_model}"
        )
...
If the image is unclear or doesn't contain medical information, set confidence < 0.5 and explain why."""
    ...
    def _has_image_data(self, state: MedSafeState) -> bool:
        ...
        return (
            'image_data' in state or
            'image_path' in state or
            'image_base64' in state
        )
        ...
        try:
            ...
            try:
                ...
                if '```json' in response_text:
                    ...
                    logger.warning("⚠️  JSON parsing failed, using manual extraction")
    ...
    def _calculate_confidence(self, parsed_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if sections and len(sections) > 0:
                ...
                section_confidences = [
                    s.get('confidence', 0.5) for s in sections
                ]
