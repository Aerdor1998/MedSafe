...
PATIENT_AGE_PATTERN = RedactionPattern(
    name="patient_age",
    pattern=re.compile(
        r"(?:idade|age|paciente)[:\s=]*\d{1,3}(?:\s*anos?)?", re.IGNORECASE
    ),
...
PATIENT_WEIGHT_PATTERN = RedactionPattern(
    name="patient_weight",
    pattern=re.compile(
        r"(?:peso|weight)[:\s=]*\d{1,3}(?:\.\d+)?(?:\s*kg)?", re.IGNORECASE
    ),
...
API_KEY_PATTERN = RedactionPattern(
    name="api_key",
    pattern=re.compile(
        r'(?:api[_-]?key|apikey|token|secret|password|senha)[:\s=]["\']*[A-Za-z0-9_\-]{8,}["\']*',
        re.IGNORECASE,
    ),
...
CONDITIONS_PATTERN = RedactionPattern(
    name="conditions",
    pattern=re.compile(
        r'"(?:conditions|comorbidades|allergies|alergias)"\s*:\s*\[[^\]]*\]',
        re.IGNORECASE,
    ),
...
class PHIRedactionFilter(logging.Filter):
    ...
    def _redact_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if any(
                kw in key_lower
                for kw in ("password", "senha", "secret", "token", "api_key", "hash")
            ):
                result[key] = "[REDACTED]"
...
def redact_sensitive_data(
    text: str, patterns: Optional[List[RedactionPattern]] = None
) -> str:
    """
...
def setup_log_redaction(
    logger: Optional[logging.Logger] = None, enabled: bool = True
) -> PHIRedactionFilter:
    """
